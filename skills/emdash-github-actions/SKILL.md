...
description: |
  Sets up GitHub Actions CI/CD workflows for EmDash plugins — TypeScript type-checking, ESLint linting, Vitest testing, npm publishing, and automated releases. Use this skill whenever a user wants to create, add, set up, or configure GitHub Actions, CI/CD, automated checks, or deployment workflows for an EmDash plugin, even without the exact phrase "GitHub Actions" — including adding automated type-checking/linting, setting up CI testing, auto-publishing to npm from GitHub, adding security/dependency scanning, wanting quality gates on PRs, or generally "add CI", "add automated checks", "set up workflows", or "automate" anything for an EmDash plugin repo. Also trigger when setting up a new EmDash plugin repo and asking for automation best practices, or when inside an EmDash plugin repository discussing automated testing, code quality, deployment, or GitHub workflows.
---
