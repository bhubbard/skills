...
description: |
  Sets up GitHub Actions CI/CD workflows for WordPress plugins — coding standards (WPCS/PHPCS), PHP/JS/CSS linting, PHPUnit testing, static analysis (PHPStan), Composer security scanning, Playground PR previews, and automated deployment to WordPress.org. Use whenever a user wants to create, add, set up, or configure GitHub Actions, CI/CD, automated checks, or deployment workflows for a WordPress plugin, even without saying "GitHub Actions" — including adding WPCS/PHPCS checks, PHP/JS/CSS linting, PHPUnit testing in CI, auto-deploying to WordPress.org, Playground PR previews, security/static analysis, or generally "add CI", "add automated checks", "set up workflows", or "automate" anything for a WP plugin repo. Also trigger for wanting to stop manual SVN deploys, wanting PR quality gates, setting up a new plugin repo and asking for automation best practices, or discussing testing/code quality/deployment/workflows inside a WordPress plugin repository.
---
